...
df.entrada <- data.frame(session_id="GsbYI",secuencia="187,104,71")
df.entrada
# Vamos a suponer que la secuencia ya exite en la tabla de secuencias solicitadas

# Recogemos la secuencia de acciones y la convertimos a vector
secuenciaSolicitada <- unlist(strsplit(as.character(df.entrada$secuencia), split=", "))
secuenciaSolicitada

# Recogemos la sesión
session_id <- df.entrada$session_id
